...
function mqtt_single_connect(brokerUrl, options) {
...
function mqtt_single_disconnect(brokerUrl, done) {
    var client = connections[brokerUrl];
...
class TasmotaMqttClient {
    constructor(tasmota_node, broker_node) {
        var self = this;

        self.node = tasmota_node;
        self.brokerUrl = broker_node.brokerurl;
        self.subscriptions = [];

        self.mqtt_client = mqtt_single_connect(self.brokerUrl, broker_node.options)
    ...
    disconnect(done) {
        ...
        this.subscriptions = [];
        mqtt_single_disconnect(this.brokerUrl, done);
    }
